...
nu_ecs_write (nu_ecs_t ecs, nu_seria_t seria)
{
    ...
    for (nu_size_t c = 0; c < ins->components.size; ++c)
    {
        ...
        dto->hash               = nu_str_hash(component_name);
    ...
    // write dtos
    nu_seria_buffer_t buffer = nu_seria_write_begin(
        seria, nu_seria_type(NU_STR("ecs_component")), dto_count);
...
nu_ecs_read (nu_ecs_t ecs, nu_seria_t seria, nu_seria_buffer_t buffer)
{
    ...
    nu__ecs_comp_dto_t dtos[256];
    nu_size_t          dto_count = nu_seria_read_begin(
        seria, nu_seria_type(NU_STR("ecs_component")), buffer);
