...
nu_controller_update (nu_controller_t controller, float dt, nu_camera_t camera)
{
    ...
    // Translation
    nu_v3_t direction = NU_VEC3_ZEROS;

    direction = nu_v3_add(
        direction,
        nu_v3_muls(nu_q4_mulv3(ctrl->rot, NU_VEC3_FORWARD), move.z));
    direction = nu_v3_add(
        direction,
        nu_v3_muls(nu_q4_mulv3(ctrl->rot, NU_VEC3_RIGHT), move.x));
    if (ctrl->mode == NU_CONTROLLER_FREEFLY)
    {
        direction = nu_v3_add(
            direction,
            nu_v3_muls(nu_q4_mulv3(ctrl->rot, NU_VEC3_UP), move.y));
    }
    else if (ctrl->mode == NU_CONTROLLER_FREEFLY_ALIGNED)
    {
        direction = nu_v3_add(direction, nu_v3_muls(NU_VEC3_UP, move.y));
    }
    else
    {
    ...
    else
    {
        ...
        ctrl->rot   = nu_q4_axis(NU_VEC3_UP, -nu_radian(ctrl->yaw));
        ctrl->rot   = nu_q4_mul(
            ctrl->rot, nu_q4_axis(NU_VEC3_RIGHT, -nu_radian(ctrl->pitch)));
    }
    ...
    {
        force = nu_v3_add(force, nu_v3_muls(NU_VEC3_DOWN, 5));
    }
    ...
    if (ctrl->mode == NU_CONTROLLER_CHARACTER)
    {
        ...
        {
            ctrl->on_ground = NU_FALSE;
            ctrl->vel       = nu_v3_muls(NU_VEC3_UP, 15);
        }
    }
    ...
    if (ctrl->mode == NU_CONTROLLER_CHARACTER)
    {
        ...
        force = nu_v3_add(
            force, nu_v3_muls(nu_v3(ctrl->vel.x, 0, ctrl->vel.z), -0.5));
    }
    else
    {
    ...
    nu_v3_t forward = nu_q4_mulv3(ctrl->rot, NU_VEC3_FORWARD);
    nu_v3_t up      = nu_v3_normalize(nu_q4_mulv3(ctrl->rot, NU_VEC3_UP));

    nu_camera_set_view(
        camera, nu_lookat(ctrl->pos, nu_v3_add(ctrl->pos, forward), up));
}
nu_m4_t
