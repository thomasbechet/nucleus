...
#if defined(_WIN32) &&                                                         \
    !defined(RGFW_X11) /* (if you're using X11 on windows some how) */

/* this name looks better */
/* plus it helps with cross-compiling because RGFW_X11 won't be accidently
 * defined */
...
*/
#define RGFW_mouseButtonPressed                                                \
  4 /*!< a mouse button has been pressed (left,middle,right)*/
#define RGFW_mouseButtonReleased                                               \
  5 /*!< a mouse button has been released (left,middle,right)*/
#define RGFW_mousePosChanged                                                   \
  6 /*!< the position of the mouse has been                                    \
       changed*/
/*! mouse event note
...
#define RGFW_dnd_init                                                          \
  35 /*!< the start of a dnd event, when the place where the file drop is      \
        known */
/* dnd data note
...
/*
NOTE : Monitor functions should be ran only as many times as needed (not in a
loop)
*/
...
typedef struct RGFW_Event {
  ...
  u32 fps; /*the current fps of the window [the fps is checked when events are
              checked]*/
  ...
  b8 inFocus; /*if the window is in focus or not (this is always true for MacOS
                 windows due to the api being weird) */
  ...
  u8 button; /*!< which mouse button has been clicked (0) left (1) middle (2)
                right OR which joystick button was pressed*/
...
typedef struct RGFW_window {
  ...
  u32 fpsCap;  /*!< the fps cap of the window should run at (change this var to
                  change the fps cap, 0 = no limit)*/
 ...
 * used */
RGFWDEF void
RGFW_setBufferSize(RGFW_area size); /* the buffer cannot be resized (by RGFW) */
...
RGFWDEF void RGFW_window_move(RGFW_window *win, RGFW_vector v /* new pos*/
);
...
#endif
RGFWDEF void RGFW_window_resize(RGFW_window *win, RGFW_area a /* new size*/
);
...
RGFWDEF void RGFW_window_setBorder(
    RGFW_window *win,
    b8 border); /* if the window should have a border or not (borderless) based
                   on bool value of `border` */

RGFWDEF void
RGFW_window_setDND(RGFW_window *win,
                   b8 allow); /* turn on / off dnd (RGFW_ALLOW_DND stil must be
...
RGFWDEF void RGFW_window_setMouseDefault(
    RGFW_window *win); /* sets the mouse to1` the default mouse image */
/*
        holds the mouse in place by moving the mouse back each time it moves
        you can still use win->event.point to see how much it moved before it
...
RGFWDEF b8 RGFW_wasPressed(
    RGFW_window *win,
    u8 key); /*!< if key was pressed (checks prev keymap only) (key code)*/
...
RGFWDEF void RGFW_clipboardFree(
    char *str); /* the string returned from RGFW_readClipboard must be freed */
...
typedef void (*RGFW_mouseNotifyfunc)(RGFW_window *win, RGFW_vector point,
                                     b8 status);
...
typedef void (*RGFW_jsButtonfunc)(RGFW_window *win, u16 joystick, u8 button,
                                  b8 pressed);
/* RGFW_jsAxisMove, the window that got the event, the joystick in question, the
 * axis values and the amount of axises */
typedef void (*RGFW_jsAxisfunc)(RGFW_window *win, u16 joystick,
                                RGFW_vector axis[2], u8 axisesCount);
...
/* OpenGL init hints */
RGFWDEF void
RGFW_setGLStencil(i32 stencil); /* set stencil buffer bit size (8 by default) */
...
RGFWDEF void
RGFW_setGLAuxBuffers(i32 auxBuffers); /* number of aux buffers (0 by default) */

/*! Set OpenGL version hint */
RGFWDEF void RGFW_setGLVersion(i32 major, i32 minor);
RGFWDEF void *
RGFW_getProcAddress(const char *procname); /* get native opengl proc address */
...
RGFWDEF void
RGFW_window_checkFPS(RGFW_window *win); /*!< updates fps / sets fps to cap (ran
                                           by RGFW_window_checkEvent)*/
...
-lopengl32 -lshell32 -lgdi32 -o RGFW.dll linux: gcc -shared RGFW.o -lX11
-lXcursor -lGL -o RGFW.so macos: gcc -shared RGFW.o -framework Foundation
...
Linux -> Only symcodes are values, (XK_0 - XK_1, XK_a - XK_z) are larger than
0xFF00, I can't find any way to work with them without making the array an
...
void RGFW_mouseNotifyfuncEMPTY(RGFW_window *win, RGFW_vector point, b8 status) {
  RGFW_UNUSED(win);
  RGFW_UNUSED(point);
  RGFW_UNUSED(status);
}
void RGFW_mouseposfuncEMPTY(RGFW_window *win, RGFW_vector point) {
  RGFW_UNUSED(win);
  RGFW_UNUSED(point);
}
void RGFW_dndInitfuncEMPTY(RGFW_window *win, RGFW_vector point) {
  RGFW_UNUSED(win);
  RGFW_UNUSED(point);
}
...
void RGFW_jsAxisfuncEMPTY(RGFW_window *win, u16 joystick, RGFW_vector axis[2],
                          u8 axisesCount) {
  RGFW_UNUSED(win);
  RGFW_UNUSED(joystick);
  RGFW_UNUSED(axis);
...
void RGFW_window_checkEvents(RGFW_window *win) {
...
void RGFW_setWindowMoveCallback(RGFW_windowmovefunc func) {
  RGFW_windowMoveCallback = func;
}
void RGFW_setWindowResizeCallback(RGFW_windowresizefunc func) {
  RGFW_windowResizeCallback = func;
}
void RGFW_setWindowQuitCallback(RGFW_windowquitfunc func) {
  RGFW_windowQuitCallback = func;
}
void RGFW_setMousePosCallback(RGFW_mouseposfunc func) {
  RGFW_mousePosCallback = func;
}
void RGFW_setWindowRefreshCallback(RGFW_windowrefreshfunc func) {
  RGFW_windowRefreshCallback = func;
}
void RGFW_setFocusCallback(RGFW_focusfunc func) { RGFW_focusCallback = func; }
void RGFW_setMouseNotifyCallBack(RGFW_mouseNotifyfunc func) {
  RGFW_mouseNotifyCallBack = func;
}
void RGFW_setDndCallback(RGFW_dndfunc func) { RGFW_dndCallback = func; }
void RGFW_setDndInitCallback(RGFW_dndInitfunc func) {
  RGFW_dndInitCallback = func;
}
void RGFW_setKeyCallback(RGFW_keyfunc func) { RGFW_keyCallback = func; }
void RGFW_setMouseButtonCallback(RGFW_mousebuttonfunc func) {
  RGFW_mouseButtonCallback = func;
}
void RGFW_setjsButtonCallback(RGFW_jsButtonfunc func) {
  RGFW_jsButtonCallback = func;
}
void RGFW_setjsAxisCallback(RGFW_jsAxisfunc func) {
...
RGFW_window *RGFW_window_basic_init(RGFW_rect rect, u16 args) {
  ...
  if (args & RGFW_CENTER)
    rect = RGFW_RECT((screenR.w - rect.w) / 2, (screenR.h - rect.h) / 2, rect.w,
                     rect.h);
...
void RGFW_window_scaleToMonitor(RGFW_window *win) {
  ...
  RGFW_window_resize(win, RGFW_AREA(((u32)monitor.scaleX) * win->r.w,
...
#ifndef RGFW_MACOS
  RGFW_window_moveMouse(win,
                        RGFW_VECTOR(win->r.x + (area.w), win->r.y + (area.h)));
#endif
}
...
u32 RGFW_isPressedJS(RGFW_window *win, u16 c, u8 button) {
  return win->src.jsPressed[c][button];
}
...
#ifndef RGFW_EGL
i32 RGFW_STENCIL = 8, RGFW_SAMPLES = 4, RGFW_STEREO = GL_FALSE,
    RGFW_AUX_BUFFERS = 0;
#else
i32 RGFW_STENCIL = 0, RGFW_SAMPLES = 0, RGFW_STEREO = GL_FALSE,
    RGFW_AUX_BUFFERS = 0;
#endif
...
u8 *RGFW_getMaxGLVersion(void) {
  ...
  static u8 version[2];
  version[0] = versionStr[0] - '0', version[1] = versionStr[2] - '0';
...
void RGFW_init_buffer(RGFW_window *win, XVisualInfo *vi) {
  ...
  OSMesaMakeCurrent(win->src.rSurf, win->buffer, GL_UNSIGNED_BYTE, win->r.w,
                    win->r.h);
#endif
...
void printFBConfigInfo(Display *display, GLXFBConfig fbConfig, int screen) {
  int attribute;
  int value;
  static int attrs[] = {GLX_BUFFER_SIZE,      GLX_LEVEL,
                        GLX_DOUBLEBUFFER,     GLX_STEREO,
                        GLX_AUX_BUFFERS,      GLX_RED_SIZE,
                        GLX_GREEN_SIZE,       GLX_BLUE_SIZE,
                        GLX_ALPHA_SIZE,       GLX_DEPTH_SIZE,
                        GLX_STENCIL_SIZE,     GLX_ACCUM_RED_SIZE,
                        GLX_ACCUM_GREEN_SIZE, GLX_ACCUM_BLUE_SIZE,
                        GLX_ACCUM_ALPHA_SIZE, None};
  ...
  if (visualInfo) {
    ...
    printf("No visual associated with this FBConfig\n");
  }

  (void)attrs;
  (void)value;
  (void)attribute;
  /* for (int i = 0; attrs[i] != None; i++) {
    attribute = attrs[i];
    if (glXGetFBConfigAttrib(display, fbConfig, attribute, &value) == 0) {
      printf("Attribute 0x%x: %d\n", attribute, value);
    } else {
      printf("Attribute 0x%x: N/A\n", attribute);
    }
  } */
}
...
RGFW_window *RGFW_createWindow(const char *name, RGFW_rect rect, u16 args) {
  ...
  Display *display = win->src.display;
  int screen = DefaultScreen(display);
  int numFBConfigs = 0;
  GLXFBConfig *fbConfigs = glXGetFBConfigs(display, screen, &numFBConfigs);
  if (!fbConfigs || numFBConfigs == 0) {
    fprintf(stderr, "No FBConfigs found\n");
  }
  ...
  for (int i = 0; i < numFBConfigs; i++) {
    printf("FBConfig %d:\n", i);
    printFBConfigInfo(display, fbConfigs[i], DefaultScreen(win->src.display));
    printf("\n");
  }
#ifdef RGFW_OPENGL
  ...
  if ((args & RGFW_NO_INIT_API) == 0) {
    ...
    win->src.rSurf = glXCreateContextAttribsARB(
  ...
  if (args & RGFW_ALLOW_DND) { /* init drag and drop atoms and turn on drag and
    ...
    XChangeProperty((Display *)win->src.display, (Window)win->src.window,
                    XdndAware, 4, 32, PropModeReplace, (u8 *)&version,
...
RGFW_Event *RGFW_window_checkEvent(RGFW_window *win) {
  ...
  if (XEventsQueued((Display *)win->src.display, QueuedAlready) +
  ...
  case KeyRelease:
    ...
    /* set event key data */
    KeySym sym = XkbKeycodeToKeysym((Display *)win->src.display, E.xkey.keycode,
  ...
  case ButtonRelease:
    ...
    RGFW_mouseButtons_prev[win->event.button] =
        RGFW_mouseButtons[win->event.button];
    ...
    if (E.xclient.message_type == XdndEnter) {
      ...
      if (list) {
        ...
        XGetWindowProperty((Display *)win->src.display, xdnd.source,
                           ...
                           &actualFormat, (unsigned long *)&count,
                           (unsigned long *)&bytesAfter, (u8 **)&formats);
      } else {
        count = 0;
...
RGFW_monitor RGFW_XCreateMonitor(i32 screen) {
  ...
  monitor.rect = RGFW_RECT(0, 0, DisplayWidth(display, screen),
...
RGFW_window *RGFW_createWindow(const char *name, RGFW_rect rect, u16 args) {
  ...
  if (!(args & RGFW_NO_BORDER)) {
    window_style |=
        WS_CAPTION | WS_SYSMENU | WS_BORDER | WS_VISIBLE | WS_MINIMIZEBOX;
    ...
    if (wglCreateContextAttribsARB != NULL) {
      ...
      if (wglChoosePixelFormatARB != NULL) {
        i32 *pixel_format_attribs =
            (i32 *)RGFW_initAttribs(args & RGFW_OPENGL_SOFTWARE);
...
#define INPUT_DEADZONE                                                         \
  (0.24f *                                                                     \
   (float)(0x7FFF)) // Default to 24% of the +/- 32767 range.   This is a
    ...
    e->axisesCount = 2;
    RGFW_vector axis1 =
        RGFW_VECTOR(state.Gamepad.sThumbLX, state.Gamepad.sThumbLY);
    RGFW_vector axis2 =
        RGFW_VECTOR(state.Gamepad.sThumbRX, state.Gamepad.sThumbRY);
...
typedef NS_ENUM(NSInteger, NSOpenGLContextParameter) {
  ...
  NSOpenGLContextParameterSurfaceOrder NS_OPENGL_ENUM_DEPRECATED(10.0, 10.14) =
      235, /* 1 param.  1 -> Above Window (default), -1 -> Below Window    */
  NSOpenGLContextParameterSurfaceOpacity NS_OPENGL_ENUM_DEPRECATED(10.0,
                                                                   10.14) =
      236, /* 1 param.  1-> Surface is opaque (default), 0 -> non-opaque   */
  NSOpenGLContextParameterSurfaceBackingSize NS_OPENGL_ENUM_DEPRECATED(
  ...
  NSOpenGLContextParameterSurfaceSurfaceVolatile API_DEPRECATED(
...
CVReturn displayCallback(CVDisplayLinkRef displayLink, const CVTimeStamp *inNow,
  ...
  RGFW_UNUSED(flagsIn)
  RGFW_UNUSED(flagsOut) RGFW_UNUSED(displayLinkContext) return kCVReturnSuccess;
}
...
#define NSOpenGLCPSurfaceOpacity 236
      NSOpenGLContext_setValues(win->src.rSurf, &opacity,
...
RGFW_vector RGFW_getGlobalMousePoint(void) {
  ...
  return RGFW_VECTOR(
      (u32)point.x, (u32)point.y); /* the point is loaded during event checks */
}
...
RGFW_Event *RGFW_window_checkEvent(RGFW_window *win) {
  ...
  NSEvent *e = (NSEvent *)((
      ...
      NSApp, eventFunc, ULONG_MAX, NULL,
...
static RGFW_monitor RGFW_NSCreateMonitor(CGDirectDisplayID display) {
  ...
  snprintf(monitor.name, 128, "%i %i %i", CGDisplayModelNumber(display),
           CGDisplayVendorNumber(display), CGDisplaySerialNumber(display));
