...
void drawLoop(
    RGFW_window *w); /* I seperate the draw loop only because it's run twice */
...
void
refreshCallback (RGFW_window *win)
{
    drawLoop(win);
}
...
main (void)
{
    RGFW_window *win = RGFW_createWindow("RGFW Example Window",
                                         RGFW_RECT(500, 500, 500, 500),
...
#endif
    RGFW_createThread(
        (RGFW_threadFunc_ptr)loop2,
        NULL); /* the function must be run after the window of this thread is
                  made for some reason (using X11) */
...
drawLoop (RGFW_window *w)
{
    ...
    RGFW_window_swapBuffers(
        w); /* NOTE(EimaMei): Rendering should always go: 1. Clear everything 2.
               Render 3. Swap buffers. Based on
...
#ifndef __APPLE__
    RGFW_window *win
        = RGFW_createWindow("subwindow", RGFW_RECT(200, 200, 200, 200), 0);
#else
